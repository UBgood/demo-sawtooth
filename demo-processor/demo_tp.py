...
# Prefix for demo is the first six hex digits of SHA-512(TF name).
sw_namespace = _hash(FAMILY_NAME.encode('utf-8'))[0:6]

class demoTransactionHandler(TransactionHandler):
    '''                                                       
    Transaction Processor class for the demo transaction family.       
    ...
    def apply(self, transaction, context):
        '''This implements the apply function for this transaction handler.
           ...
           a single transaction for the demo transaction family.   
        '''                                                   
        
        # Get the payload and extract demo-specific information.
...
def main():
    '''Entry-point function for the demo transaction processor.'''
    setup_loggers()
    try:
        # Register the transaction handler and start it.
        processor = TransactionProcessor(url='tcp://validator:4004')

        handler = demoTransactionHandler(sw_namespace)
